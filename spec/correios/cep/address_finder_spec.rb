...
describe Correios::CEP::AddressFinder do
  describe '#find' do
    context 'when zipcode is valid' do
      ...
      let(:address) { Correios::CEP::Address.new }
      ...
      it 'returns address' do
        expect(subject.find(zipcode)).to eq address
      end
    end
        ...
        values.each do |text, value|
          context "with #{text}" do
            it "raises ArgumentError with '#{error_message}' error message" do
              expect { subject.find(value) }.to raise_error(ArgumentError, error_message)
            end
          end
        end
      end
  ...
  describe '#get' do
    context 'when zipcode is valid' do
      ...
      it 'returns address in hash format' do
